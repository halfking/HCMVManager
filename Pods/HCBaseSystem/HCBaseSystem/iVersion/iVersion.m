...
static NSString *const iVersioniOSAppStoreURLFormat = @"https://itunes.apple.com/app/id%u";
...
@property (nonatomic, copy) NSString *versionDetailsP;
...
@synthesize versionDetailsP = _versionDetails;
...
{
    if (!self.versionDetailsP)
    {
        if (self.viewedVersionDetails)
        {
            self.versionDetailsP = [self versionDetails:self.applicationVersion inDict:[self localVersionsDict]];
        }
        else 
        {
            self.versionDetailsP = [self versionDetailsSince:self.lastVersion inDict:[self localVersionsDict]];
        }
    }
    return self.versionDetailsP;
}
...
- (void)downloadedVersionsData
{
    ...
    {
//        SEL selector = NSSelectorFromString(@"iVersionVersionCheckFailed:");
...
//            [self.delegate performSelector:selector withObject:self.downloadError];
//        }
        return;
    }
    ...
    if (details)
    {
        SEL selector = NSSelectorFromString(@"iVersionDetectedNewVersion:details:");
...
//            [self.delegate performSelector:selector withObject:mostRecentVersion withObject:details];
//        }
...
- (void)checkForNewVersionInBackground
{
    @synchronized (self)
    {
        @autoreleasepool
        {
            ...
            NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:iTunesServiceURL] cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:REQUEST_TIMEOUT];
            ...
            if (data)
            {
                ...
                {
                    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:self.remoteVersionsPlistURL]
...
- (void)checkIfNewVersion
{
    ...
    if (self.lastVersion != nil || self.showOnFirstLaunch || self.debug)
    {
        if ([self.applicationVersion compareVersion:self.lastVersion] == NSOrderedDescending || self.debug)
        {
            ...
            //get version details
            BOOL showDetails = !!self.versionDetailsP;
            if (showDetails && [self.delegate respondsToSelector:@selector(iVersionShouldDisplayCurrentVersionDetails:)])
            {
                showDetails = [self.delegate iVersionShouldDisplayCurrentVersionDetails:self.versionDetailsP];
            }
            ...
            if (showDetails && !self.visibleLocalAlert)
            {
                ...
                self.visibleLocalAlert = [NSAlert alertWithMessageText:self.inThisVersionTitle
                                                       ...
                                                       alternateButton:nil
                                                           otherButton:nil
                                             informativeTextWithFormat:@"%@", self.versionDetailsP];
...
- (void)openAppPageWhenAppStoreLaunched
{
    ...
    while (GetNextProcess(&psn) == noErr)
    {
        ...
        {
            //open app page
            [[NSWorkspace sharedWorkspace] performSelector:@selector(openURL:) withObject:[self updateURLA] afterDelay:MAC_APP_STORE_REFRESH_DELAY];
            CFRelease(cfDict);
            return;
        }
        CFRelease(cfDict);
...
- (void)openAppPageInAppStore
{
    [[NSWorkspace sharedWorkspace] openURL:[self updateURLA] ];
